...
void test_do_rename()
{
        dbg(DBG_PRINT, "(GRADING2C) starting test_do_rename() !\n");
        do_open("F1.txt",O_CREAT);
        do_rename("F1.txt","F2.txt");
        dbg(DBG_PRINT, "(GRADING2C) test_do_rename() SUCCESSFUL !\n");
}
        ...
        {
            KASSERT(kshell != NULL);
            dbg(DBG_PRINT, "(GRADING1B): do_foo() is invoked, argc = %d, argv = 0x%08x\n",
        ...
        {
        	KASSERT(kshell != NULL);
        	dbg(DBG_PRINT, "(GRADING1B): do_foo1() is invoked, argc = %d, argv = 0x%08x\n",
            argc, (unsigned int)argv);
        	sunghan_test(0, NULL);
        	return 0;
        }
        ...
        {
        	KASSERT(kshell != NULL);
        	dbg(DBG_PRINT, "(GRADING1C): do_foo2() is invoked, argc = %d, argv = 0x%08x\n",
            argc, (unsigned int)argv);
        	sunghan_deadlock_test(0, NULL);
        	return 0;
        }

        int do_foo3(kshell_t *kshell, int argc, char **argv)
        {
        	KASSERT(kshell != NULL);
        	dbg(DBG_PRINT, "(GRADING2B): do_foo3() is invoked, argc = %d, argv = 0x%08x\n",
            argc, (unsigned int)argv);
        	vfstest_main(1, NULL);
        	return 0;
        }

        int do_foo4(kshell_t *kshell, int argc, char **argv)
        {
        	KASSERT(kshell != NULL);
        	dbg(DBG_PRINT, "(GRADING2C): do_foo4() is invoked, argc = %d, argv = 0x%08x\n",
        	argc, (unsigned int)argv);
        	test_do_rename();
            return 0;
        }
...
initproc_run(int arg1, void *arg2)
{
		...
		kshell_add_command("vfstest", do_foo3, "invoke VFS_test cases...");
		kshell_add_command("dorenametest", do_foo4, "invoke test_do_rename test case...");
