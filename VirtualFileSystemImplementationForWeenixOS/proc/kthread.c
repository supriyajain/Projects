...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
       /* NOT_YET_IMPLEMENTED("PROCS: kthread_create"); */
		KASSERT(NULL != p);
		dbgq(DBG_CORE, "(GRADING1 3.a) Process is not NULL\n");
...
kthread_cancel(kthread_t *kthr, void *retval)
{
		...
		KASSERT(NULL!=kthr);
		dbgq(DBG_CORE, "(GRADING1 3.b) Thread is not NULL\n");
		if(kthr!=curthr)
		{
			sched_cancel(kthr);
...
kthread_exit(void *retval)
{
		...
		dbgq(DBG_CORE, "(GRADING1 3.c) Queue is empty\n");
		KASSERT(!curthr->kt_qlink.l_next && !curthr->kt_qlink.l_prev);
		dbgq(DBG_CORE, "(GRADING1 3.c) Queue is empty\n");
		KASSERT(curthr->kt_proc == curproc);
		dbgq(DBG_CORE, "(GRADING1 3.c) current thread's process is current process\n");
