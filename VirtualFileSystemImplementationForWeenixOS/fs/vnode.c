...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
		...
		int data=0;
		KASSERT(file);
		dbg(DBG_PRINT, "(GRADING2A 1.a) file object is checked\n");
	    KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
	    dbg(DBG_PRINT, "(GRADING2A 1.a) file is either byte type or block type device\n");
		if(S_ISBLK(file->vn_mode))
	 		 return -ENOTSUP;
	     if(S_ISCHR(file->vn_mode))
	     {
	    	 KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
	    	 dbg(DBG_PRINT, "(GRADING2A 1.a) read method for the device type exists\n");
	    	 data=file->vn_cdev->cd_ops->read(file->vn_cdev,(int)offset,buf,(int)count);
	     }
	     return data;
}
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
		...
		int data=0;
		KASSERT(file);
		dbg(DBG_PRINT, "(GRADING2A 1.b) file object is checked\n");
		KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
		dbg(DBG_PRINT, "(GRADING2A 1.b) file is either byte type or block type device\n");
		if(S_ISBLK(file->vn_mode))
			return -ENOTSUP;
		if(S_ISCHR(file->vn_mode))
		{
			KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
		    dbg(DBG_PRINT, "(GRADING2A 1.b) write method for the device type exists\n");
		    data=file->vn_cdev->cd_ops->write(file->vn_cdev,(int)offset,buf,(int)count);
		}
		return data;
}
